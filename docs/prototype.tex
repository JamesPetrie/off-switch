...
Hardware designs are typically written in a hardware description language (HDL), simulated to verify correctness, then synthesized to physical circuits. HardCaml is an HDL embedded in OCaml that supports simulation and testing before committing to silicon. Using HardCaml allows the design to be validated against test cases and iterated on quickly.
...
The state machine (Figure \ref{fig:statemachine}) orchestrates the authorization flow:
\begin{enumerate}[nosep]
...
\draw[arrow, rounded corners] (update.south) -- ++(0, -0.4) -- ++(1.2, 0) -- ++(0, -2.6) -- ++(-4.5, 0) -- (publish.south);
\node[font=\scriptsize] at (3.2, -5.3) {invalid};
