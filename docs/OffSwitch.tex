...
To validate the security block design presented in Section \ref{sec:primary_design}, we implemented a single-block prototype in RTL (register-transfer level) hardware description language. The prototype demonstrates that the core cryptographic authorization mechanism is implementable with standard digital logic, and that the security properties described in the paper can be enforced in hardware. Source code is available at \url{https://github.com/jamesbpetrie/security_block}.
