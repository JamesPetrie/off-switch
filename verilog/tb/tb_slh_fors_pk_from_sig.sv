module tb (
    ...
    logic                         start;
    logic [127:0]                 pk_seed;
    logic [HMSG_FORS_BITS-1:0]    fors_message;
    ...
    logic [127:0]                 expected_fors_key[0:0];
    int                           tests_passed;

    slh_fors_pk_from_sig dut (
        .clk,
        .rst_n,
        .start,
        .pk_seed,
    ...
    task automatic run_case(
        input logic corrupt_first_secret,
        ...
        begin
            ...
            for (element_index = 0;
                 element_index < FORS_ELEMENTS;
                 element_index++) begin
                if ((element_index % 11) == 5) begin
                    @(negedge clk);
                end
                while (!element_ready) @(negedge clk);
                next_element = signature_elements[element_index + 1];
                if (corrupt_first_secret && (element_index == 0)) begin
            ...
            if (expect_match
                && (public_key[255:128] !== expected_fors_key[0])) begin
                $fatal(1, "%s mismatch: got=%032x expected=%032x",
                    label_text, public_key[255:128], expected_fors_key[0]);
            end
            if (!expect_match
                && (public_key[255:128] === expected_fors_key[0])) begin
                $fatal(1, "%s corruption did not change the FORS key",
                    label_text);
            end
    ...
    initial begin
        ...
        $readmemh({VECTOR_DIR, "/fors_public_key.hex"}, expected_fors_key);

        pk_seed      = public_key_elements[0];
        fors_message = hmsg_value[0][M_BYTES*8-1 -: HMSG_FORS_BITS];
        ...
        run_case(1'b0, 1'b1, "real liboqs FORS signature");
        run_case(1'b1, 1'b0, "corrupted revealed FORS secret");

        if (tests_passed != 2) begin
            $fatal(1, "unexpected pass count: %0d", tests_passed);
        end
        $display("All %0d real-vector FORS recovery tests passed.",
            tests_passed);
        $finish;
    end
