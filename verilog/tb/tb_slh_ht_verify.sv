module tb (
    ...
    task automatic run_case(
        input logic corrupt_first_wots_value,
        ...
        begin
            ...
            for (element_index = 0;
                 element_index < HT_ELEMENTS;
                 element_index++) begin
                if ((element_index % 13) == 6) begin
                    @(negedge clk);
                end
                while (!element_ready) @(negedge clk);
                next_element = signature_elements[
                    HT_FIRST_ELEMENT + element_index];
                if (corrupt_first_wots_value && (element_index == 0)) begin
            ...
            if (expect_match && (root[255:128] !== public_key_elements[1])) begin
                $fatal(1, "%s mismatch: got=%032x expected=%032x",
                    label_text, root[255:128], public_key_elements[1]);
            end
            if (!expect_match && (root[255:128] === public_key_elements[1])) begin
                $fatal(1, "%s corruption did not change the hypertree root",
                    label_text);
            end
            ...
            tests_passed++;
            $display("PASS [%s] HT_ROOT=%032x", label_text, root[255:128]);
            @(posedge clk);
        end
    endtask
    ...
    initial begin
        ...
        run_case(1'b0, 1'b1, "real liboqs seven-layer hypertree");
        run_case(1'b1, 1'b0, "corrupted hypertree WOTS+ value");

        if (tests_passed != 2) begin
            $fatal(1, "unexpected pass count: %0d", tests_passed);
        end
        $display("All %0d real-vector hypertree recovery tests passed.",
            tests_passed);
        $finish;
    end
